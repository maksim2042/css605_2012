...
def Play():
        ...
        def playGame():
                ...
                while len(pattern)==0:
                        ...
                        geneMax=max([x[0] for x in p1Fitness])
                        print 'generation',generation,'mean score:',geneMean, ', max score:',geneMax, '  p2:',p2.r
                        print '--------------------'
                        pattern=[x[1] for x in p1Fitness if x[0]>=p2.r]
                        if len(pattern)>0:
                                print "winning strategy detected...."
                                print 'this strategy consists of....', len(p1.population[pattern[0]]),'moves'
                                print p1.population[pattern[0]]
                                print '------------------------'
                                print 'opponent strategy....'
                                print 'this strategy consists of....', len(p2.plays),'moves'
                                print p2.plays
                                print '------------------------'
                        selection()
                        newpopulation()
